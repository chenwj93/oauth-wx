...
func ParseStringFromStruct(m interface{}) string {
	s, _ := json.Marshal(m)
	return string(s)
}
...
//created by cwj 2018-03-16
// O2 = O1
func Assignment(O1 interface{}, O2 interface{}) error{
	dataJson, err := json.Marshal(O1)
	if err != nil {
		return err
	}
	err = json.Unmarshal(dataJson, O2)
	return err
}
...
func SimplifyStringMap(m map[string]interface{}, l int) string{
	...
	for key, value := range m{
		...
		vs := string(vb)
		if len(vs) > l{
			sb.WriteString(fmt.Sprintf("%s:%s +++ too long ", key, vs[:l]))
		} else {
			sb.WriteString(fmt.Sprintf("%s:%s  ", key, vs))
		}
	}
	sb.WriteString("]")
	return sb.String()
