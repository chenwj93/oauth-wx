...
func debugLogQueies(alias *alias, operaton, query string, t time.Time, err error, args ...interface{}) {
	...
	if len(cons) > 0 {
		...
		// modified by cwj on 2018-05-17
		// 修改日志输出，当参数长度超过200，则只输出前200个字符
		var consP bytes.Buffer
		for index, con := range cons{
			if len(con) > 200{
				consP.WriteString(con[:200])
				consP.WriteString("+++++ too long")
			}else {
				consP.WriteString(con)
			}
			if index < len(cons) -1{
				consP.WriteString("`, `")
			}
		}
		con += fmt.Sprintf(" - `%s`", consP.String())
	}
	if err != nil {
		con += " - " + err.Error()
	}
